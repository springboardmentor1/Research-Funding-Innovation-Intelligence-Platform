...
export default function Profile() {
  ...
  useEffect(() => {
    if (user?.role === "administrator") return; // skip API call for admins
    axiosClient
  ...
  // ── Role guard (all hooks already declared above) ───────────────────────
  // Administrators have no research profile — redirect them to their dashboard.
  ...
  if (loading) {
    return (
      <Layout maxWidth="max-w-4xl" className="py-16">
        <Loading message="Loading your research profile…" className="text-center text-sm text-ink-900/50" />
      </Layout>
    );
  }
          ...
          <Card className="mt-8">
            ...
            {showPubForm && (
              <form onSubmit={handleAddPublication} className="mt-4 grid gap-3 rounded-lg border border-ink-900/8 bg-surface-50 p-4 sm:grid-cols-2">
                <input className="field-input sm:col-span-2" placeholder="Publication title" value={newPublication.title} onChange={(e) => setNewPublication((p) => ({ ...p, title: e.target.value }))} required />
                <input className="field-input" placeholder="Authors" value={newPublication.authors} onChange={(e) => setNewPublication((p) => ({ ...p, authors: e.target.value }))} />
                <input className="field-input" placeholder="Journal / Conference" value={newPublication.journal} onChange={(e) => setNewPublication((p) => ({ ...p, journal: e.target.value }))} />
                <input type="date" className="field-input" value={newPublication.publication_date} onChange={(e) => setNewPublication((p) => ({ ...p, publication_date: e.target.value }))} />
                <input className="field-input" placeholder="DOI" value={newPublication.doi} onChange={(e) => setNewPublication((p) => ({ ...p, doi: e.target.value }))} />
                <input className="field-input sm:col-span-2" placeholder="URL" value={newPublication.url} onChange={(e) => setNewPublication((p) => ({ ...p, url: e.target.value }))} />
            ...
            <div className="mt-4 divide-y divide-ink-900/5">
              {publications.length === 0 && (
                <EmptyState message="No publications added yet." className="py-4 text-sm text-ink-900/45" />
              )}
          ...
          <Card className="mt-8">
            ...
            {showPatentForm && (
              <form onSubmit={handleAddPatent} className="mt-4 grid gap-3 rounded-lg border border-ink-900/8 bg-surface-50 p-4 sm:grid-cols-2">
                <input className="field-input sm:col-span-2" placeholder="Patent title" value={newPatent.title} onChange={(e) => setNewPatent((p) => ({ ...p, title: e.target.value }))} required />
                <input className="field-input" placeholder="Patent number" value={newPatent.patent_number} onChange={(e) => setNewPatent((p) => ({ ...p, patent_number: e.target.value }))} />
                <input className="field-input" placeholder="Assignee" value={newPatent.assignee} onChange={(e) => setNewPatent((p) => ({ ...p, assignee: e.target.value }))} />
                <input type="date" className="field-input" value={newPatent.filing_date} onChange={(e) => setNewPatent((p) => ({ ...p, filing_date: e.target.value }))} />
                <input className="field-input" placeholder="Technology domain" value={newPatent.technology_domain} onChange={(e) => setNewPatent((p) => ({ ...p, technology_domain: e.target.value }))} />
            ...
            <div className="mt-4 divide-y divide-ink-900/5">
              {patents.length === 0 && (
                <EmptyState message="No patents added yet." className="py-4 text-sm text-ink-900/45" />
              )}
              {patents.map((pat) => (
