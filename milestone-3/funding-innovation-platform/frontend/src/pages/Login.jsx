...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Strict role-tab enforcement: if the account role doesn't match the
      // selected tab, immediately invalidate the session and show an error.
      if (!roleConfig.allowedRoles.includes(returnedRole)) {
        logout();
        setForm({ email: "", password: "" });
        setServerError(roleConfig.errorMessage);
        setTimeout(() => emailRef.current?.focus(), 0);
        return;
      }

      // Honour any redirect the router stored (e.g. a protected-route bounce),
      // otherwise send to the role-appropriate default landing page.
      const destination = location.state?.from?.pathname || roleConfig.redirect;
