...
export default function Notifications() {
  ...
  return (
    <Layout maxWidth="max-w-3xl">
      ...
      <Card className="divide-y divide-ink-900/5 p-0">
        {loading && <Loading className="px-6 py-10 text-center text-sm text-ink-900/40" />}
        {!loading && data?.items.length === 0 && (
          <EmptyState message="No notifications to show." className="px-6 py-10 text-center text-sm text-ink-900/40" />
        )}
        {!loading &&
          data?.items.map((n) => (
