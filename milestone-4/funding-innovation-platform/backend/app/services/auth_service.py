...
class AuthService:
    ...
    async def register(self, payload: UserRegister) -> TokenResponse:
        ...
        # Administrator role cannot normally be self-assigned at registration
        # time. The single exception is a one-time bootstrap: if the system
        # has zero administrators yet, the very first request that asks for
        # role=administrator is honored so the platform has an initial admin
        # to work with. The instant that account is created, has_any_admin()
        # becomes True and this path is permanently closed — no separate
        # flag, env var, or manual step required.
        if payload.role == UserRole.ADMINISTRATOR and not self.repo.has_any_admin():
            role = UserRole.ADMINISTRATOR
            logger.warning("Bootstrapping the first Administrator account: %s", payload.email)
        else:
        ...
        if role == UserRole.ADMINISTRATOR:
            await log_activity(
                ...
                user_id=str(user.id),
                email=user.email,
                metadata={"note": "First Administrator account created via one-time bootstrap path."},
            )
