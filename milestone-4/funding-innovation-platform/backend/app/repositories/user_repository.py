...
class UserRepository:
    ...
    def has_any_admin(self) -> bool:
        """Used exclusively to gate the one-time Administrator bootstrap path
        in AuthService.register(). Returns True the moment any Administrator
        account exists, permanently closing that path from then on."""
        stmt = select(User.id).where(User.role == UserRole.ADMINISTRATOR).limit(1)
