...
## PostgreSQL & AI Integration Best Practices Followed

1. **Use of UUIDs**: All primary keys (except the master taxonomy lookup `research_interests` which uses integer series for simplicity and join efficiency) use UUIDv4 to avoid predictable integer sequencing and support multi-tenant distributed environments.
2. **Strict Referential Integrity**: Cascade deletes (`ON DELETE CASCADE`) clean up junction table entries when parents are removed. A set null (`ON DELETE SET NULL`) is applied for institutional changes to ensure profile records remain intact if an institution profile is updated.
3. **Automated Timestamps**: Database triggers dynamically update the `updated_at` columns across audit-critical tables.
4. **Optimized Indexing**:
   - Indexes on foreign keys to prevent sequential table scans during frequent join statements.
   - B-Tree indexes on timeline dates (`published_date`, `filing_date`, `deadline`) for quick sorting.
   - Indexes on categorical columns like `status` and matching `score` values to speed up filter routines.
5. **AI Vector/Embedding Ready**:
   - The modular design allows easily attaching a pgvector column (`vector` data type) to `research_profiles`, `funding_opportunities`, `publications`, or `patents` if semantic similarity search is added in the future.
   - Symmetrically indexing interests (`research_interests`) across profiles, opportunities, publications, and patents allows fast graph-based matching algorithms.
