...
def calculate_match_score(
    ...
    # Realistic AI matching score formula (Base 72% + 5% per matching keyword boost, capped at 98%)
    base_score = 0.72
    boost = min(0.26, len(intersection) * 0.05) if intersection else 0.0
    score_placeholder = base_score + boost

    # Rationale description
    explanation = (
        f"Matched {score_placeholder*100:.1f}% based on domain alignment and "
        f"{len(intersection)} overlapping keywords: ({', '.join(intersection) if intersection else 'none'})."
    )

    opportunity_result = opportunity.copy()
    opportunity_result["match_score"] = round(score_placeholder * 100, 2)  # Return as 0-100 percentage scale
